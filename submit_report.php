...
// Get employee ID
$getEmployee = "SELECT id FROM lgntable WHERE username = '$username'";
...
if ($empResult && mysqli_num_rows($empResult) > 0) {
    ...
    echo "<p style='color: red;'>Error: Could not find employee with username '$username'</p>";
    exit;
}
...
$query = "SELECT p.project_number, p.project_objective
          FROM project p 
        ...
        <form method="POST" action="submit_report.php">
            ...
            <select name="project_id" required>
                ...
                <?php while ($row = mysqli_fetch_assoc($result)) { ?>
                    <option value="<?= $row['project_number'] ?>">
                        <?= $row['project_number'] ?> - <?= htmlspecialchars($row['project_objective']) ?>
                    </option>
                <?php } ?>
            </select>
